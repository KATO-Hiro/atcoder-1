...
int main() {
    ...
    combination Z(sum_f+2);

    // dp[i][j] := i 番目の英小文字まで見た時，同じ文字が j 箇所で隣り合う場合の数
    VVM dp(N+1,VM(sum_f+1,0)); dp[0][0] = 1;
    ll cum_f = 0;
    FOR(i,1,N+1) {
        ll f = F[i-1];
        // k := i 番目の英小文字を f[i] 個挿入する際の分割数
        // l := j 箇所のうち次の文字 (分割済) を挿入する箇所の数
        REP(j,cum_f+1) FOR(k,1,f+1) REP(l,min(j,k)+1) {  // 
            dp[i][j-l+(f-k)] += dp[i-1][j] * Z.C(j,l) * Z.C(cum_f+1-j,k-l) * Z.C(f-1,k-1);
        }
        cum_f += f;
    }
    mint ans = dp[N][0];
